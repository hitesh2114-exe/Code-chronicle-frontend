...
function DisplayIssue() {
  ...
  const handletoggle = async () => {
    try {
      const response = await axios.post(
        `http://localhost:3000/issue/togglestatus/${id}`
      );
      const refreshed = await axios.get(`http://localhost:3000/issue/${id}`);
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:3000/repo/deleteIssueFromRepo/${issue.repository._id}`,
        { issueId: id }
      );
